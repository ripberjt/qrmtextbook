...
<<echo=FALSE>>=
ds.temp<-na.omit(subset(ds, select=c("glbcc","age","education","income","ideol","gender")))
@ 
<<>>=
...
<<tidy=TRUE>>=
logit2 <- glm(glbcc ~ age+gender+education+income+ideol, family=binomial(), data=ds.temp)
summary(logit2)
...
<<tidy=TRUE>>=
L <- logit2$coef[1]+logit2$coef[2]*mean(ds.temp$age)+logit2$coef[3]*mean(ds.temp$gender)+
  logit2$coef[4]*mean(ds.temp$education)+logit2$coef[5]*mean(ds.temp$income)+
  logit2$coef[6]*(ds.temp$ideol)
P <- 1/(1+exp(-L))
...
<<echo=2:8,results="hide",tidy=TRUE>>=
pdf(file="logitplot.pdf")
data <- data.frame(age=mean(ds.temp$age), gender=mean(ds.temp$gender), education=mean(ds.temp$education),
                   income=mean(ds.temp$income), ideol=1:7)
...
plot(plogis(preds[["fit"]]) ~ data[["ideol"]], type="l",
     xlab="Ideology", ylab="GHG cause GCC", ylim=c(min(lower),
                                                   max(upper)))
