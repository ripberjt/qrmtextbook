...
<<echo=FALSE>>=
ds.temp<-na.omit(subset(ds, select=c("glbcc_risk","age","education","income","ideol","gender")))
@ 
...
<<>>=
stan.ds<-data.frame(scale(ds.temp[,c("glbcc_risk","age","education","income","ideol","gender")]))
