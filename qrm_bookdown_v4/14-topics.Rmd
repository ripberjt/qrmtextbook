...
```{r dum, echo=FALSE, warning=FALSE, fig.cap="Dummy Intercept Variables"}
set.seed(5)
y <-rnorm(5)
x <-1:5
mod <- lm(y ~ x)
...
ggplot(df) + 
  geom_segment(aes(x = 1, y = 0.65, xend = 4.5, yend = 0), linetype = "dashed", color = "green", size = 1) +
  geom_segment(aes(x = 1, y = 0.3, xend = 4.5, yend = -0.35), linetype = "dashed", color = "blue", size = 1) +
  annotate("text", x = 4.8, y = 0., label = expression("X"["2,0"]), color = "red", fontface = 1) +
  annotate("text", x = 4.8, y = -0.35, label = expression("X"["2,1"]), color = "red", fontface = 1) +
...
```{r dumin, echo=FALSE, warning=FALSE, fig.cap="Illustration of Slope Interaction"}
set.seed(5)
y <-rnorm(5)
x <-1:5
mod <- lm(y ~ x)
...
ggplot(df) + 
  geom_segment(aes(x = 1, y = 0, xend = 4.5, yend = 0.25), linetype = "dashed", color = "green", size = 1) +
  geom_segment(aes(x = 1, y = 0, xend = 4.5, yend = -0.4), linetype = "dashed", color = "blue", size = 1) +
  annotate("text", x = 4.8, y = 0.25, label = expression("X"["2,0"]), color = "red", fontface = 1) +
  annotate("text", x = 4.8, y = -0.4, label = expression("X"["2,1"]), color = "red", fontface = 1) +
  lims(y = c(-1, 1), x = c(1, 5)) + 
  labs(x = expression("X"[1]), y = "Y") +
  theme(axis.text = element_blank(),
        axis.ticks = element_blank(),
