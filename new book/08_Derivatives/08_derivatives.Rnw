...
Once a critical point is identified, the next step is to determine whether that point is a minima or a maxima. The most straightforward way to do this is to identify the x,y coordinates and plot. This can be done in \texttt{R}, as we will show using the function $y=f(x)=(x^{2}-4x+5)$. The plot is shown in Figure \ref{fig:crit}. 
<<echo=2:5,results="hide">>=
pdf("crit.pdf")
x <- c(-5:5)
x
...
\noindent The model to be estimated is expressed as $Y=\hat{\beta}+\hat{\beta}X+/epsilon$. 
...
Using \texttt{R}, this can be accomplished in a few steps. First create a vector of values for \textit{x} and \textit{y} (note that we chose these values arbitrarily for the purpose of this example). 
...
\noindent To see the relationship, we can produce a scatterplot of \textit{x} and \textit{y} and add our regression line, as shown in Figure \ref{fig:regex}. So, for each unit increase in $x$, $y$ increases by \Sexpr{B} and when $x$ is $0$, $y$ is equal to \Sexpr{A}.  
