...
This can be calculated in \texttt{R} using the full model with ideology. 
...
The pseudo $R^2$ of the model is \Sexpr{pseudoR2}. Note that the psuedo $R^2$ is only an approximation of explained variance, and should be used in combination with other measures of fit such as AIC.
...
Therefore, for each 1-unit increase in the ideology scale (i.e., becoming more conservative), the odds of believing that climate change is human caused decrease by \Sexpr{exp(logit2$coef[6])}. 
